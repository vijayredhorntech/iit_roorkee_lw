<div class="mt-6 w-full border-[1px] border-t-[4px] border-primary/20 border-t-primary bg-white flex gap-2 flex-col shadow-lg shadow-gray-300 mt-6">
    ...
    <div class="bg-primary/10 px-4 py-2 border-b-[2px] border-b-primary/20 flex justify-between">
        <span class="font-semibold text-primary text-xl">Departments</span>
        <span wire:click="showDepartmentModel" class="text-sm bg-primary/80 px-4 py-1 rounded-[3px] rounded-tr-[8px] rounded-bl-[8px] font-semibold border-[2px] border-primary/80 text-white hover:text-white hover:bg-primary hover:border-ternary/30 transition ease-in duration-2000 cursor-pointer">
    ...
    <div class="w-full overflow-x-auto p-4">
        <table class="w-full border-[2px] border-secondary/40 border-collapse" wire:loading.class="opacity-25">
            ...
            @forelse($departments as $departmentData)
                <tr class="hover:bg-secondary/10 cursor-pointer transition ease-in duration-2000">
                    ...
                    <td class="border-[2px] border-secondary/40 px-4 py-1.5 text-ternary/80 font-medium text-sm">
                        <span><i class="fa fa-tag mr-1 text-success"></i> {{ $departmentData->title }}</span> <br>
                    </td>
                    <td class="border-[2px] border-secondary/40 px-4 py-1.5 text-ternary/80 font-medium text-sm">
                        <div class="flex gap-2">
                            <button wire:click="editDepartment({{ $departmentData->id }})" title="Edit Department"
                            ...
                            <button wire:click="deleteDepartment({{ $departmentData->id }})"
                                    wire:confirm="Are you sure you want to delete this department?" title="Delete"
            ...
            @empty
                <tr>
                    <td colspan="3" class="border-[2px] border-secondary/40 px-4 py-1.5 text-ternary/80 font-medium text-sm text-center">No departments found</td>
                </tr>
            @endforelse
        </table>
    </div>
    ...
    @if($showDepartmentModal)
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-6">
            <div class="bg-white p-6 rounded-lg shadow-xl max-w-md w-full">
                <h3 class="text-lg font-semibold mb-4 text-primary">{{ $isEditing ? 'Edit Department' : 'Add New Department' }}</h3>
                <form wire:submit="saveDepartment">
                    ...
                    <div class="flex justify-end space-x-3 mt-4">
                        ...
                        <button type="submit"
                            ...
                            <span wire:loading.remove wire:target="saveDepartment">{{ $isEditing ? 'Update' : 'Save' }}</span>
                            <span wire:loading wire:target="saveDepartment">{{ $isEditing ? 'Updating...' : 'Saving...' }} <i class="fas fa-hourglass-half fa-spin ml-2"></i></span>
                        </button>
                    </div>
                </form>
            </div>
